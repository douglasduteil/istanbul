...
(function (isNode) {
    ...
    var SYNTAX,
        ...
        SM = isNode ? require('source-map') : null,
    ...
    function Instrumenter(options) {
        ...
        if (this.opts.transpiler){
            this.transpiler = require('istanbul-' + this.opts.transpiler);
        }
    ...
    Instrumenter.prototype = {
        ...
        instrumentSync: function (code, filename) {
            ...
            if (this.transpiler){
                result = this.transpiler(code);
                this.transpilerSourceMap = new SM.SourceMapConsumer(result.map);
                code = result.code;
            }
        ...
        coverStatement: function (node, walker) {
            ...
            if (this.transpilerSourceMap){
                loc = {
                    start: this.transpilerSourceMap.originalPositionFor(location.start),
                    end: this.transpilerSourceMap.originalPositionFor(location.end)
                };
            }
            ...
            if (node.type === SYNTAX.FunctionDeclaration.name) {
                sName = this.statementName(loc, 1);
            } else {
                sName = this.statementName(loc);
        ...
        functionName: function (node, line, location) {
            ...
            if (this.transpilerSourceMap){
                loc = {
                    start:  this.transpilerSourceMap.originalPositionFor(clone('start')),
                    end:  this.transpilerSourceMap.originalPositionFor(clone('end'))
                };
        ...
        branchName: function (type, startLine, pathLocations) {
            ...
            if (this.transpilerSourceMap){
                startLine = locations[0].start.line;
            }
        ...
        ifBranchInjector: function (node, walker) {
            ...
            if (this.transpilerSourceMap) {
                var startLocation = this.transpilerSourceMap.originalPositionFor(node.loc.start);
                if (!startLocation.line) {
                    return;
                }
            }
